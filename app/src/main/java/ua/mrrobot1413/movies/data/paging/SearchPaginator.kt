...
class SearchPaginator<Item, Key>(
    private val initialKey: Key,
    private var isFirstLoad: Boolean = true,
    private inline val firstLoad: (Boolean) -> Unit,
    private inline val onLoadUpdated: (Boolean) -> Unit,
    private inline val onRequest: suspend (query: String, nextKey: Key, type: ListType) -> Result<List<Item>>,
    ...
    override suspend fun loadNextItems(type: ListType) {
        ...
        onLoadUpdated(true)
        val result = onRequest(query ?: "", currKey, type)
        println(result)
